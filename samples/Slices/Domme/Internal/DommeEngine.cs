...
internal sealed class DommeEngine
{

    public Result<DommeAssessment> Evaluate(CaseId caseId, IReadOnlyList<ProvisionRef> provisions, string factum)
    {
        if (string.IsNullOrWhiteSpace(factum))
            return Result<DommeAssessment>.Failure("empty factum");
        ...
        return Result<DommeAssessment>.Success(new DommeAssessment(caseId, band, normalized, notes));
    }
